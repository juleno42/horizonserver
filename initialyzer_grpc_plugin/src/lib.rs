...
pub struct InitializerGrpcPlugin {
    inner: Box<dyn SimplePlugin>,
}

impl InitializerGrpcPlugin {
    pub fn new() -> Self {
        let config = GrpcBridgeConfig::new("initializer_grpc_plugin", "http://172.22.0.1:50052")
            .with_version("2.4.0")
            ...
            .with_client_events(vec![
                ...
                ("box50cm", "spawn"),("notifications","periodic_notification")
            ])
            ...
            .with_plugin_events(vec![
                ...
                ("Persistance_grpc_plugin","plugin_acknowledged")
            ])
...
impl SimplePlugin for InitializerGrpcPlugin {
    fn name(&self) -> &str {
        self.inner.name()
    }
...
create_simple_plugin!(InitializerGrpcPlugin);