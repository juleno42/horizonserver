...
pub struct EcoGrpcPlugin {
    inner: Box<dyn SimplePlugin>,
}

impl EcoGrpcPlugin {
    pub fn new() -> Self {
        let config = GrpcBridgeConfig::new("eco_grpc_plugin", "http://172.22.0.1:50052")
            .with_version("2.4.0")
...
impl SimplePlugin for EcoGrpcPlugin {
    fn name(&self) -> &str {
        self.inner.name()
    }
...
create_simple_plugin!(EcoGrpcPlugin);